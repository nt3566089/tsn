 ...
 * 入荷検品ワーク登録ロジッククラス
 */
public class ReceiveInspectionInsertLogic extends AbstractWmsLogic {
	...
	public void insert(TStoreRecordH header, List<TStoreRecordB> bodyList, ErrorStatus errSts) {
		...
		// [ON推-品向-790] 2015/05/18 別明細で同一商品を同じ場所に入荷した際に同一入庫ラベルNoをつける YOKOSUKA [S]
		...
		// [ON推-品向-790] 2015/05/18 別明細で同一商品を同じ場所に入荷した際に同一入庫ラベルNoをつける YOKOSUKA [E]
	 ...
	 * <pre>
	 * 引数を条件に【入荷検品関連データ取得.入荷予定情報取得】で入荷予定ボディのデータを取得し、
	 * 入庫数合計と予定数を比較する。
	 * 取得した全データで入庫数合計が予定数以上の場合、
	 * 入荷予定ヘッダの入荷納品ステータスを"01"(完納)で更新する。
	 * 入荷予定ヘッダの入荷ステータスが"01"(未入荷)又は"03"(入荷済)の場合、
	 * 入荷ステータスを"02"(入荷中)で更新する。
	 *
	 * 【入荷検品関連データ取得.入荷予定情報取得】
	 * ・{@link ReceiveInspectionSelectLogic#selectReceivePlanListData(TReceivePlanH, MProduct, BUser) 入荷予定情報取得 メソッド}を呼び出す。
	 * </pre>
	...
	public void received(TReceivePlanH planH) {
		...
		if (planBodyList.size() ==0) {
			...
			//[HT1.5-CT1-030] ここでの更新処理は入荷済に更新しない(但し、納品フラグは更新する) 2015/06/25 KAI UPDATE START
			/**
			if (planBodyList.get(0).isReceiveStatus$03()) {
				// ===== 入荷予定ヘッダの更新実行 =====
				receiveH.setReceiveStatus_$03();
				if (finishFlg) {
			...
			*/
			//未入荷か入荷済なら入荷中に更新に変更 2015/07/01 KAI UPDATE START
			//if (receiveH.isReceiveStatus$01()){
			if (receiveH.isReceiveStatus$01() || receiveH.isReceiveStatus$03()){
			//未入荷か入荷済なら入荷中に更新に変更 2015/07/01 KAI UPDATE END
				receiveH.setReceiveStatus_$02();
			}
			if (finishFlg) {
				receiveH.setReceiveDeliveryStatus_$01();
			}
			//[HT1.5-CT1-030] ここでの更新処理は入荷済に更新しない(但し、納品フラグは更新する) 2015/06/25 KAI UPDATE END
