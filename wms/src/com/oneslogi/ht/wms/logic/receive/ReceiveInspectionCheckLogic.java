 ...
 * 入荷検品チェックロジッククラス
 */
public class ReceiveInspectionCheckLogic extends AbstractWmsLogic {
	...
	public TReceivePlanH checkReceiveSlipNo(TReceivePlanH tReceivePlanH, String msgFlg) {
		//[HT1.5-CT1-036] 再開時の削除チェックで使い回したいためメッセージフラグを追加 2015/06/25 KAI UPDATE END
		...
		if (lstTReceivePlanH.size() == 0) {
			//[HT1.5-CT1-036] 再開時の削除チェックで使い回したいためメッセージフラグでメッセージ変更 2015/06/25 KAI UPDATE START
			if (msgFlg.equals("1")) {
				// 対象の伝票が削除されました。
			...
			//[HT1.5-CT1-036] 再開時の削除チェックで使い回したいためメッセージフラグでメッセージ変更 2015/06/25 KAI UPDATE END
			return null;
		}
	 ...
	 * <pre>
	 * 引数を条件に【入荷検品関連データ取得.予定数合計を取得】を実行し、
	 * 入荷予定ボディの予定数合計を取得する。
	 * 予定数合計が０の場合、入荷予定数０は不可のエラーをエラーステータスに設定する。
	 *
	 * 【入荷検品関連データ取得.予定数合計を取得】
	 * ・{@link ReceiveInspectionSelectLogic#selectReceivePlanData(TReceivePlanH, MProduct, BUser) 予定数合計を取得メソッド}を呼び出す。
	 * </pre>
	...
	public TReceivePlanB checkJanProduct(TReceivePlanH tReceivePlanH, MProduct mProduct, BUser bUser) {
		...
		// すべての検品作業が完了しました ====
		//if (tReceivePlanB.isReceiveStatus$03()) {
		//	// すべての検品作業が完了しました
	...
	public void checkOverPlanNum(TReceivePlanB planB) {
		...
		if (CU.equals(strCheck, "2")) {
			//[HT1.5-CT1-042] 数量超過時のエラーメッセージを変更 2015/06/25 KAI UPDATE START
			...
			//[HT1.5-CT1-042] 数量超過時のエラーメッセージを変更 2015/06/25 KAI UPDATE END
			return;
		}
	 ...
	 * <h2>過入荷の数量チェックを行う。</h2>
	 * <pre>
	 * 引数の「入庫実績ボディの入庫数合計＋検品数」が予定数を超えている場合、
	 * 過入荷エラーをエラーステータスに設定する。
	 * </pre>
	 * @param planB 入荷予定ボディ：入庫実績ボディの入庫数合計・予定数
	 * @param dto 出荷検品Dto：検品数
	 */
	public void checkOverPlanBPlanNum(TReceivePlanB planB, ReceiveInspectionDto dto) {

		BigDecimal planNum = planB.getPlanNum();
		BigDecimal finishedNum = planB.getSumStoreNum();
		if(WCC.isLessThan(planNum, WCC.add(finishedNum, dto.getStoreNum()))){
			getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_QTY_OVER_ERROR);
			return;
		}

//		TStoreRecordBCB tStoreRecordBCB = tStoreRecordBBhv.newConditionBean();
//		tStoreRecordBCB.setupSelect_TReceivePlanB();
//		tStoreRecordBCB.query().setReceivePlanBId_Equal(planB.getReceivePlanBId());
//		List<TStoreRecordB> lstStoreB = tStoreRecordBBhv.selectList(tStoreRecordBCB);
//		if (lstStoreB != null) {
//			long finishTotal = 0l;
//			for (TStoreRecordB storeB : lstStoreB) {
//				finishTotal += storeB.getStoreNum();
//			}
//			long plan = planB.getPlanNum();
//			long remain = plan - finishTotal;
//			if (remain < dto.getStoreNum()) {
//				getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_QTY_OVER_ERROR);
//				return;
//			}
//		}
	...
	/**
	 * <h2>予定数と入庫数の差異チェックを行う。</h2>
	 *  <pre>
	 * 引数を条件に入荷予定情報を取得する。
	 * 商品単位で入庫実績ボディの入庫数合計値を抽出し、予定数と差異がある場合は、trueを返す。
	 *
	 * </pre>
	 ...
	 * 入荷ステータスが"02"(入荷中)で、入庫実績ボディの更新日が最新であるデータの入力区分が"30"(HT)の場合、trueを返す。
	 *
	 * </pre>
	 * @param tReceivePlanH 入荷予定ヘッダ：荷主ID・センタID・入荷予定ヘッダID・WMS入荷伝票No.
