 ...
 * 入荷検品関連データ取得ロジッククラス
 */
public class ReceiveInspectionSelectLogic extends AbstractWmsLogic {
	...
	//public MParam getMParam(MClient mClient, MCenter mCenter) {
	...
	//2015/05/14 [ON推-品向-811] WEBHT情報はIDに変更する YOKOSUKA [E]
		...
		//2015/05/14 [ON推-品向-811] WEBHT情報はIDに変更する YOKOSUKA [S]
		//mParamCB.query().queryMClientCenter().setClientId_Equal(mClient.getClientId());
		//mParamCB.query().queryMClientCenter().setCenterId_Equal(mCenter.getCenterId());
	...
	/**
	 * <h2>ロケーションマスタから仮ロケ情報を取得する。</h2>
	 * <pre>
	 * 引数の条件で、ロケーションマスタから入庫仮ロケーションのデータを取得する。
	 * </pre>
	 * @param mLocation ロケーションタマスタ：センタID
	 * @return MLocation:ロケーションタマスタ
	 */
	public MLocation getPlanLocation(MLocation mLocation) {
		//パラメータ情報の取得
		MLocationCB mLocationCB = mLocationBhv.newMyConditionBean();
		mLocationCB.query().setCenterId_Equal(mLocation.getCenterId());
	 ...
	 * <h2>入荷予定情報を取得する。</h2>
	 * <pre>
	 *  引数を条件に入荷予定関連データ及び入庫実績ボディの入庫数合計を以下の並び順で取得する。
	 * ・センタ間移動フラグが「1：センタ間移動」の場合
	 *   行No. 昇順
	 * ・上記以外の場合
	 *   入荷ステータス 昇順、入荷ヘッダID 昇順
	...
	public List<TReceivePlanB> selectReceivePlanListData(TReceivePlanH tReceivePlanH, final MProduct mProduct, BUser bUser) {
		...
		if (bUser != null) {
			//[]在庫区分名称は入荷予定ヘッダの在庫区分IDから取得 2015/09/11 KAI UPDATE START
			//tReceivePlanBCB.setupSelect_TReceivePlanH().withMProcessType().withMStockType().withVDict(bUser.getCultureId());;
			tReceivePlanBCB.setupSelect_TReceivePlanH().withMStockType().withVDict(bUser.getCultureId());
			//[]在庫区分名称は入荷予定ヘッダの在庫区分IDから取得 2015/09/11 KAI UPDATE END
		}
		...
		if (mProduct != null) {
			tReceivePlanBCB.orScopeQuery(new OrQuery<TReceivePlanBCB>() {
				public void query(TReceivePlanBCB orCB) {
					// [ON推-品向-987]商品情報再取得時に、JANCDがNULLだと動きがおかしくなる不具合対応 2015/10/29 Yokosuka [S]
					if( mProduct.getJanCd() == null ){
					...
					// [ON推-品向-987]商品情報再取得時に、JANCDがNULLだと動きがおかしくなる不具合対応 2015/10/29 Yokosuka [E]
				}
			});
...
//[C-CWMS-0049]ケース入数表示をケース荷姿の入数に修正 2016/1/13 chou 削除 End
	 ...
	 * 予定数合計、入庫数合計、残数合計（過入荷の場合：０）を計算する。
	 * 各合計数とJAN/商品コードの最小ステータスを設定して返却する。
	 *
	 * </pre>
	 * @param tReceivePlanH 入荷予定ヘッダ：センタID・荷主ID・入荷予定ヘッダID・WMS入荷伝票No.・センタ間移動フラグ
	 * @param mProduct 商品マスタ：JANCD・商品CD
...
//		Long totalLeftPlanNum = 0l;
//		if(lstTemp!=null){
//			Long tempSumPlanNum =0l;
//			Long tempSumStoreNum = 0l;
//			for (TReceivePlanB tempPlanB : lstTReceivePlanB) {
//				tempSumPlanNum = tempSumPlanNum + tempPlanB.getPlanNum();
//				tempSumStoreNum = tempSumStoreNum + tempPlanB.getSumStoreNum();
//			}
//			// 過入荷場合、０を設定する
//			if (tempSumPlanNum - tempSumStoreNum > 0) {
//				totalLeftPlanNum = tempSumPlanNum - tempSumStoreNum;
//			}
//		}
