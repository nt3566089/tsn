...
/**
 * 検品格納(詳細)センタ移動ロット・期限日選択画面Resourceクラスです。
 *
 */
@Path("/handy/ReceiveInspecStoreCenterMoveLotLimitDt")
@HandyErrorReturnPath(returnURL = "ReceiveInspecStoreCenterMoveLotLimitDt/display")
public class ReceiveInspecStoreCenterMoveLotLimitDtResource extends HandyResourceBase{
	@Inject
	...
	public static void display(HandyResourceBase resource, HandyCommonUtil util) throws Exception {
		...
		// 画面初期設定
		resource.initScreen("ReceiveInspecStoreCenterMoveLotLimitDtHT");
		// 非表示ボタンの設定
		resource.setHideButton(ButtonInfo.builder().url("ReceiveInspecStoreCenterMoveLotLimitDt/send").build());
		//フッタボタン（左）設定
		resource.setOptionButton1(
				ButtonInfo.builder()
				.url("ReceiveInspecStoreCenterMoveLotLimitDt/prev")
				.itemCd("prev")
		...
		resource.setOptionButton2(
				ButtonInfo.builder()
				.url("ReceiveInspecStoreCenterMoveLotLimitDt/next")
				.itemCd("next")
				.type(BUTTON_URL_JUMP_WITH_DATA)
		...
		//送信設定
		resource.setSendButton(ButtonInfo.builder().url("ReceiveInspecStoreCenterMoveLotLimitDt/send").itemCd("send").build());
		// [#2854] HT国際化対応(画面マスタ連携) 2017.11.02 wangqi End

		//キャンセル確認画面へ遷移
		util.setTransURL(HT_URI_BASE + "/wms/receive/ReceiveInspecStoreCenterMoveLotLimitDt.jsp", ses);
	}
	/**
	 * 前時にコールされるメソッドです。<br>
	 * 前情報を表示します。
	...
	public void send() throws Exception {
		...
		BigDecimal planNum = selectPlanB.getPlanNum();
		BigDecimal finishedNum = selectPlanB.getSumStoreNum();
		...
		if (dto.getLabelKbn().equals("1")) {
			...
			// [#7175][OSS] 検品格納のクラスに変更 2020.02.03 tsuboi Start
			// 汎用ラベルNO入力確認画面へ遷移する
			ReceiveInspecStoreLabelInputResource.load(this, util);
			// [#7175][OSS] 検品格納のクラスに変更 2020.02.03 tsuboi End
		}
	}
