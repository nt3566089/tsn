...
import com.oneslogi.ht.wms.dto.receive.ReceiveInspectionLimitDtDto;
...
/**
 * 入荷検品(詳細)期限日入力画面Resourceクラスです。
 *
 */
@Path("/handy/ReceiveInspectionLimitDt")
@HandyErrorReturnPath(returnURL = "ReceiveInspectionLimitDt/display")
public class ReceiveInspectionLimitDtResource extends HandyResourceBase {
	...
	public static void display(HandyResourceBase resource, HandyCommonUtil util) throws Exception {
		...
		// 画面初期設定
		resource.initScreen("ReceiveInspectionLimitDtHT");

		// 非表示ボタンｍの設定
		resource.setHideButton(ButtonInfo.builder().url("ReceiveInspectionLimitDt/send").build());

		//戻る設定
		resource.setReturnButton(ButtonInfo.builder().url("ReceiveInspectionQty/display").itemCd("return").build());
		//送信設定
		resource.setSendButton(ButtonInfo.builder().url("ReceiveInspectionLimitDt/send").itemCd("send").build());

		// [#2854] HT国際化対応(画面マスタ連携) 2017.10.30 潘 End

		//キャンセル確認画面へ遷移
		util.setTransURL(HT_URI_BASE + "/wms/receive/ReceiveInspectionLimitDt.jsp", ses);
	...
	@GET
	@Path("/send")
	public void send(@BeanParam ReceiveInspectionLimitDtDto bean) throws Exception {
