...
import com.oneslogi.ht.wms.dto.receive.ReceiveInspecStoreLotLimitDtDto;
...
/**
 * 検品格納(詳細)ロット・期限日入力画面Resourceクラスです。
 *
 */
@Path("/handy/ReceiveInspecStoreLotLimitDt")
@HandyErrorReturnPath(returnURL = "ReceiveInspecStoreLotLimitDt/display")
public class ReceiveInspecStoreLotLimitDtResource extends HandyResourceBase {
	...
	public static void display(HandyResourceBase resource, HandyCommonUtil util) throws Exception {
		...
		// 画面初期設定
		resource.initScreen("ReceiveInspecStoreLotLimitDtHT");
		// 非表示ボタンの設定
		resource.setHideButton(ButtonInfo.builder().url("ReceiveInspecStoreLotLimitDt/send").build());
		//戻る設定
		resource.setReturnButton(ButtonInfo.builder().url("ReceiveInspecStoreQty/display").itemCd("return").build());
		//送信設定
		resource.setSendButton(ButtonInfo.builder().url("ReceiveInspecStoreLotLimitDt/send").itemCd("send").build());
		// [#2854] HT国際化対応(画面マスタ連携) 2017.11.02 wangqi End

		//キャンセル確認画面へ遷移
		util.setTransURL(HT_URI_BASE + "/wms/receive/ReceiveInspecStoreLotLimitDt.jsp", ses);
	...
	@GET
	@Path("/send")
	public void send(@BeanParam ReceiveInspecStoreLotLimitDtDto bean) throws Exception {
