...
import com.oneslogi.ht.wms.dto.receive.ReceiveInspectionLotLimitDtDto;
...
/**
 * 入荷検品(詳細)ロット・期限日入力画面Resourceクラスです。
 *
 */
@Path("/handy/ReceiveInspectionLotLimitDt")
@HandyErrorReturnPath(returnURL = "ReceiveInspectionLotLimitDt/display")
public class ReceiveInspectionLotLimitDtResource extends HandyResourceBase {
	...
	public static void load(HandyResourceBase resource, HandyCommonUtil util) throws Exception {
		...
		if (errorManager.size() > 0) {
			resource.sendErrorScreen(util.getMessage(errorManager), "", "ReceiveInspectionQty/display");
			errorManager.clear();
			return;
		}
	...
	public static void display(HandyResourceBase resource, HandyCommonUtil util) throws Exception {
		...
		// 画面初期設定
		resource.initScreen("ReceiveInspectionLotLimitDtHT");

		// 非表示ボタンの設定
		resource.setHideButton(ButtonInfo.builder().url("ReceiveInspectionLotLimitDt/send").build());

		//戻る設定
		resource.setReturnButton(ButtonInfo.builder().url("ReceiveInspectionQty/display").itemCd("return").build());
		//送信設定
		resource.setSendButton(ButtonInfo.builder().url("ReceiveInspectionLotLimitDt/send").itemCd("send").build());

		// [#2854] HT国際化対応(画面マスタ連携) 2017.10.30 潘 End

		//キャンセル確認画面へ遷移
		util.setTransURL(HT_URI_BASE + "/wms/receive/ReceiveInspectionLotLimitDt.jsp", ses);
	...
	@GET
	@Path("/send")
	public void send(@BeanParam ReceiveInspectionLotLimitDtDto bean) throws Exception {
		...
		tReceivePlanBDto.setPlanLot(bean.getLot());
...
//		}
		...
		tStoreRecordB.setLot(bean.getLot());
