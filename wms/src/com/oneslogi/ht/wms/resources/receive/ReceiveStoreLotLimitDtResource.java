...
import com.oneslogi.ht.wms.dto.receive.ReceiveStoreLotLimitDtDto;
...
/**
 * 入庫格納(ロット・期限日入力)画面Resourceクラスです。
 *
 */
@Path("/handy/ReceiveStoreLotLimitDt")
@HandyErrorReturnPath(returnURL = "ReceiveStoreLotLimitDt/display")
public class ReceiveStoreLotLimitDtResource extends HandyResourceBase {
	...
	public static void load(HandyResourceBase resource, HandyCommonUtil util) throws Exception {
		...
		wHtReceiveStoreDto.setLotId(null);
		wHtReceiveStoreDto.setLot("");
	...
	public static void display(HandyResourceBase resource, HandyCommonUtil util) throws Exception {
		...
		resource.initScreen("ReceiveStoreLotLimitDtHT");
		// 非表示ボタンの設定
		resource.setHideButton(ButtonInfo.builder().url("ReceiveStoreLotLimitDt/send").build());
		//フッタボタン（左）設定
		resource.setReturnButton(ButtonInfo.builder().url("ReceiveStoreJanCd/display").itemCd("return").build());
		//フッタボタン（右）設定
		resource.setSendButton(ButtonInfo.builder().url("ReceiveStoreLotLimitDt/send").itemCd("send").build());
		// [#2854] HT国際化対応(画面マスタ連携) 2017.11.02  ライ MOD END
		//ロット・期限日入力へ遷移
		util.setTransURL(HT_URI_BASE + "/wms/receive/ReceiveStoreLotLimitDt.jsp", ses);
	}

	/**
	 * 送信時にコールされるメソッドです。<br>
	 * 入力チェックを行い、数量入力画面に遷移します。
	 * @param bean ロット・期限日入力画面情報を保持DTO
	 * @throws Exception
	 */
	@GET
	@Path("/send")
	public void send(@BeanParam ReceiveStoreLotLimitDtDto bean) throws Exception {
		...
		wHtReceiveStoreDto.setLot(bean.getLot());
...
//		}
		...
		//入力したロット、期限日が入庫実績データに存在するかチェックを行う
		...
		if (tStoreRecordBList.size() == 0) {
			...
			getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_STORE_LOT_LIMIT_DT_NOT_FOUND_ERROR);
			return;
		}
		...
		//入力したロット、期限日の入庫数チェックを行う
		if (WCC.isZero(lSumStoreNum)) {
			//該当のロット、期限日の入庫数が0です。
			getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_STORE_LOT_LIMIT_DT_QTY_ZERO_ERROR);
			return;
		}

		//入力したロット、期限日の入庫格納フラグが全て格納済かチェックを行う
		if (blStoreFlg == false) {
			//該当のロット、期限日が全て格納済です。
			getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_STORE_LOT_LIMIT_DT_STORED_ERROR);
			return;
		}

		//入力したロット、期限日で取得した入庫実績データから仮ロケに入荷した在庫データが存在するかチェックを行う
		...
		if (tStockInoutList.size() == 0) {
			...
			getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_STORE_LOT_LIMIT_DT_STOCK_NOT_FOUND_ERROR);
			return;
		}
		...
		if (WCC.isZero(lChargeNum)) {
			...
			getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_STORE_LOT_LIMIT_DT_STOCK_QTY_ZERO_ERROR);
			return;
		}
