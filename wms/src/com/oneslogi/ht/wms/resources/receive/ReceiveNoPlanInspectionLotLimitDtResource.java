...
import com.oneslogi.ht.wms.dto.receive.ReceiveNoPlanInspectionLotLimitDtDto;
...
/**
 * 予定無し入荷(詳細)ロット・期限日入力画面Resourceクラスです。
 *
 */
@Path("/handy/ReceiveNoPlanInspectionLotLimitDt")
@HandyErrorReturnPath(returnURL = "ReceiveNoPlanInspectionLotLimitDt/display")
public class ReceiveNoPlanInspectionLotLimitDtResource extends HandyResourceBase {
	...
	public static void display(HandyResourceBase resource, HandyCommonUtil util) throws Exception {
		...
		// 画面初期設定
		resource.initScreen("ReceiveNoPlanInspectionLotLimitDtHT");
		// 非表示ボタンの設定
		resource.setHideButton(ButtonInfo.builder().url("ReceiveNoPlanInspectionLotLimitDt/send").build());

		//戻る設定
		resource.setReturnButton(ButtonInfo.builder().url("ReceiveNoPlanInspectionQty/display").itemCd("return").build());
		//送信設定
		resource.setSendButton(ButtonInfo.builder().url("ReceiveNoPlanInspectionLotLimitDt/send").itemCd("send").build());
		// [#2854] HT国際化対応(画面マスタ連携) 2017.11.01 LSW End

		//キャンセル確認画面へ遷移
		util.setTransURL(HT_URI_BASE + "/wms/receive/ReceiveNoPlanInspectionLotLimitDt.jsp", ses);
	...
	@GET
	@Path("/send")
	public void send(@BeanParam ReceiveNoPlanInspectionLotLimitDtDto bean) throws Exception {
