...
import com.oneslogi.ht.wms.dto.receive.ReceiveStoreLotDto;
...
/**
 * 入庫格納(ロット入力)画面Resourceクラスです。
 *
 */
@Path("/handy/ReceiveStoreLot")
@HandyErrorReturnPath(returnURL = "ReceiveStoreLot/display")
public class ReceiveStoreLotResource extends HandyResourceBase {
	...
	public static void display(HandyResourceBase resource, HandyCommonUtil util) throws Exception {
		...
		resource.initScreen("ReceiveStoreLotHT");
		// 非表示ボタンの設定
		resource.setHideButton(ButtonInfo.builder().url("ReceiveStoreLot/send").build());

		//フッタボタン（左）設定
		resource.setReturnButton(ButtonInfo.builder().url("ReceiveStoreJanCd/display").itemCd("return").build());
		//フッタボタン（右）設定
		resource.setSendButton(ButtonInfo.builder().url("ReceiveStoreLot/send").itemCd("send").build());
		// [#2854] HT国際化対応(画面マスタ連携) 2017.11.01  ライ MOD END
		//ロット入力画面へ遷移
		util.setTransURL(HT_URI_BASE + "/wms/receive/ReceiveStoreLot.jsp", ses);
	}

	/**
	 * 送信時にコールされるメソッドです。<br>
	 * 入力チェックを行い、数量入力画面に遷移します。
	 * @param bean ロット入力画面情報を保持DTO
	 * @throws Exception
	 */
	@GET
	@Path("/send")
	public void send(@BeanParam ReceiveStoreLotDto bean) throws Exception {
...
//		}
		...
		if (tStoreRecordBList.size() == 0) {
			...
			getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_STORE_LOT_NOT_FOUND_ERROR);
			return;
		}
		...
		if (WCC.isZero(lSumStoreNum)) {
			//該当のロットの入庫数が0です。
			getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_STORE_LOT_QTY_ZERO_ERROR);
			return;
		}

		//入力したロットの入庫格納フラグが全て格納済かチェックを行う
		if (blStoreFlg == false) {
			//該当のロットが全て格納済です。
			getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_STORE_LOT_STORED_ERROR);
			return;
		}

		//入力したロットで取得した入庫実績データから仮ロケに入荷した在庫データが存在するかチェックを行う
		...
		if (tStockInoutList.size() == 0) {
			...
			getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_STORE_LOT_STOCK_NOT_FOUND_ERROR);
			return;
		}
		...
		if (WCC.isZero(lChargeNum)) {
			...
			getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_RECEIVE_STORE_LOT_STOCK_QTY_ZERO_ERROR);
			return;
		}
