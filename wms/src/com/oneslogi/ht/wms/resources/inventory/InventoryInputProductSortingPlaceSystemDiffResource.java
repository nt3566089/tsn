...
/**
 * ◆ｼｽﾃﾑ数量差異確認画面Resourceクラス
 *
 */
@Path("/handy/InventoryInputProductSortingPlaceSystemDiff")
@HandyErrorReturnPath(returnURL = "InventoryInputProductSortingPlaceSystemDiff/display")
public class InventoryInputProductSortingPlaceSystemDiffResource extends HandyResourceBase {
    ...
    public static void display(HandyResourceBase resource, HandyCommonUtil util, InventoryInputProductSortingPlaceLogic inventoryInputProductSortingPlaceLogic) throws Exception {
        ...
        // 2.1.(1)在庫調査入力[仕分場]情報よりｼｽﾃﾑ数量差異確認(InventoryInputProductSortingPlaceSystemDiff)の画面初期設定を行う。
        resource.initScreen("InventoryInputProductSortingPlaceSystemDiffHT");
        resource.setHeaderColor("yellow");

        // 2.1.(2)戻るボタン設定を行う。
        resource.setReturnButton(
                ButtonInfo.builder().url("InventoryInputProductSortingPlaceSystemDiff/return").itemCd("return").build());

        // 2.1.(3)確定ボタン設定を行う。
        resource.setSendButton(
                ButtonInfo.builder().url("InventoryInputProductSortingPlaceSystemDiff/send").itemCd("send").build());

        /*
         * 3.画面表示(display)
         */
        // 3.1.(1)ｼｽﾃﾑ数量差異確認(InventoryInputProductSortingPlaceSystemDiff)を表示する。
        util.setTransURL(HT_URI_BASE + "/wms/inventory/InventoryInputProductSortingPlaceSystemDiff.jsp", ses);
    }
    ...
    public void send() throws Exception {
        ...
        inventoryInputProductSortingPlaceLogic.outPrintln(dto, "E07:◆ｼｽﾃﾑ数量差異確認");
    }
