...
import com.oneslogi.ht.wms.dto.inventory.InventoryInputProductLimitDtDto;
import com.oneslogi.ht.wms.util.HandyCommonUtil;
import com.oneslogi.wms.WmsMessageConst;
import com.oneslogi.wms.core.WCC;
//import com.oneslogi.ht.wms.dto.shipping.InventoryLotLimitDtDto;
import com.oneslogi.wms.logic.common.LotLimitCheckLogic;
import com.oneslogi.wms.logic.common.LotLogic;

/**
 * 商品棚卸入力(期限日入力)画面Resourceクラスです。
 *
 */
@Path("/handy/InventoryInputProductLimitDt")
@HandyErrorReturnPath(returnURL = "InventoryInputProductLimitDt/display")
public class InventoryInputProductLimitDtResource extends HandyResourceBase {
	...
	public static void display(HandyResourceBase resource, HandyCommonUtil util) throws Exception {
		...
		resource.initScreen("InventoryInputProductLimitDtHT");
		// 非表示ボタンの設定
		resource.setHideButton(
				ButtonInfo.builder()
				.url("InventoryInputProductLimitDt/send").build());
		// オプションボタン１の設定
		resource.setOptionButton1(
				ButtonInfo.builder()
				.url("InventoryInputProductList/load?backUrl=InventoryInputProductLimitDt/display")
				.itemCd("option1").build());
		//フッタボタン（左）設定
		resource.setReturnButton(
				ButtonInfo.builder()
				.url("InventoryInputProductJanCd/display").itemCd("return").build());
		//フッタボタン（右）設定
		resource.setSendButton(
				ButtonInfo.builder()
				.url("InventoryInputProductLimitDt/send").itemCd("send").build());
		// [#2854] HT国際化対応(画面マスタ連携) 2017.10.30  ライ MOD END
		//JAN入力画面へ遷移
		util.setTransURL(HT_URI_BASE + "/wms/inventory/InventoryInputProductLimitDt.jsp",ses);
	}

	/**
	 * 送信時にコールされるメソッドです。<br>
	 * 入力チェックを行い、商品棚卸入力(数量入力)画面に遷移します。
	 * @param bean 商品棚卸入力(期限日入力)画面情報を保持DTO
	 * @throws Exception
	 */
	@GET
	@Path("/send")
	public void send(@BeanParam InventoryInputProductLimitDtDto bean) throws Exception {
