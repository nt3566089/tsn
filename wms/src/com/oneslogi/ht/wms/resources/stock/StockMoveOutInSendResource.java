 ...
 *
 */
@Path("/handy/StockMoveOutInSend")
@HandyErrorReturnPath(returnURL = "StockMoveOutInSend/display")
public class StockMoveOutInSendResource extends HandyResourceBase {
	...
	@Inject
	private StockMoveOutSelectLogic outSelectLogic;
	...
	public static void display(HandyResourceBase resource, HandyCommonUtil util) throws Exception {
		...
		resource.initScreen("StockMoveOutInSendHT");

		//フッタボタン（左）設定
		resource.setFooterLeftButton(ButtonInfo.builder()
				.url("StockMoveOutInLoc/display")
				.itemCd("no")
		...
		//送信確認画面へ遷移
		util.setTransURL(HT_URI_BASE + "/wms/stock/StockMoveOutInSend.jsp", ses);
	}
	...
	public void send() throws Exception {
		...
		List<TMoveInstB> lstTMoveInstB = tMoveInstBDtoMapper.mappingToEntityList(dto.getLstTMoveInstBDto());
		for (int i = 0; i < lstTMoveInstB.size(); i++) {
			TMoveInstB  tMoveInstB = lstTMoveInstB.get(i);
			...
			TMoveInstH header = tMoveInstHDtoMapper.mappingToEntity(dto.getLstTMoveInstBDto().get(i).getTMoveInstH());
			TMoveInstB bodyTo = tMoveInstBDtoMapper.mappingToEntity(dto.getLstTMoveInstBDto().get(i));
			...
			try {
				...
				// [C-CWMS-0050]  全数在庫移動際補充ロケ件対応 2016.2.17 nayzaw Start
				...
				// [C-CWMS-0050] 全数在庫移動際補充ロケ件対応 2016.2.17 nayzaw End
		...
		//ロケ入力画面を遷移する
        StockMoveOutLocResource.load(this, util, centerLogic, clientLogic, warehouseLogic,outSelectLogic);
	}
