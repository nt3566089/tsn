...
import com.oneslogi.wms.dto.stock.TotalStockListDto;
import com.oneslogi.wms.logic.common.CenterLogic;
import com.oneslogi.wms.logic.common.ClientLogic;
import com.oneslogi.wms.logic.common.CustomerLogic;
import com.oneslogi.wms.logic.common.StockTypeLogic;
import com.oneslogi.wms.logic.stock.TotalStockListLogic;
import com.oneslogi.wms.resources.AbstractWmsResource;

/**
 * トータル在庫問合せのリソースクラス。
 */
@Path("/stock/totalStockList")
public class TotalStockListResource extends AbstractWmsResource {
	...
	// ===== 使用ロジッククラス =====
	@Inject
	private TotalStockListLogic totalStockListLogic;
	@Inject
	 ...
	 * ・画面用DTOの作成
	 * </pre>
	 * @return TotalStockListDto 定点発注一覧画面用DTO
	 */
	@GET
	@Path("/init")
	public TotalStockListDto init() {
		// 画面用DTO作成
		TotalStockListDto totalStockListDto = new TotalStockListDto();
		return totalStockListDto;
	}

	/**
	 * <h2>トータル在庫問合せデータ取得。</h2>
	 * <pre>
	 ...
	 * 【パラメータの使用項目】
	 * </pre>
	 * @param totalStockListDto 検索条件
	 * @return PagingResultBean
	 */
	@GET
	@Path("/search")
	public TotalStockListDto search(TotalStockListDto totalStockListDto) throws Exception {
		// ===== 初期化 =====
		TotalStockListDto result = new TotalStockListDto();

		// [#594] ユーザに紐づくセンタ・荷主で絞込みを修正(不要な変数削除) 2017.01.18 kawana

		TStockDtoMapper mapper = new TStockDtoMapper();
		TStock tStock = mapper.mappingToEntity(totalStockListDto.data.head);
		...
		// [#2237] ページングを削除し検索結果の上限チェックを追加 2017.08.14 kawana Start
		// [#594] ユーザに紐づくセンタ・荷主で絞込みを修正 2017.01.18 kawana Start

		// ===== 一覧データ取得 =====

		PagingResultBean<TStock> tStocklist = totalStockListLogic.select(tStock, mCenter, mProduct, tLot, errRetSts(StatusCode.SEARCH_DATA_ERROR));
		...
		// [#2237] ページングを削除し検索結果の上限チェックを追加 2017.08.14 kawana End
