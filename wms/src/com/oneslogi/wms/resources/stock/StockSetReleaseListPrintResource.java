...
/**
 * セット解除指示書発行に必要な各種情報を取得するクラス。
 */
@Path("/stock/stockSetReleaseListPrint")
public class StockSetReleaseListPrintResource extends AbstractWmsResource {

	private static final String REPORT_CD = "StockSetReleaseList";
	private static final String PGM_CD = "StockSetReleaseListPrint";
	...
	@GET
	@Path("/init")
	public StockSetReleaseListPrintDto init() {
		StockSetReleaseListPrintDto stockSetReleaseListPrintDto = new StockSetReleaseListPrintDto();

		// 発行帳票を指定
		stockSetReleaseListPrintDto.printBasicData.reportId = reportLogic.getReportId(REPORT_CD);

		return stockSetReleaseListPrintDto;
	}

	/**
	 * <h2>セット解除指示書発行リスト発行。</h2>
	 * <pre>
	 ...
	 * ・・セット解除指示書発行リスト出力フラグ
	 * </pre>
	 * @param dto セット解除指示書発行リスト画面用DTO
	 * @return stockSetReleaseListPrintDto セット解除指示書発行リスト画面用DTO
	 * @throws Exception
	 */
	@GET
	@Path("/print")
	public StockSetReleaseListPrintDto print(final StockSetReleaseListPrintDto dto) throws Exception {
		...
		if(!CU.isNullOrEmpty(tMoveInstB.getMProduct().getProductCd())){
			...
			if (product == null) {
				...
				// 親商品存在チェック
				stockSetReleaseListPrintLogic.parentIdCheck(product.getProductId(), errRetSts(StatusCode.PRODUCT_NOT_FOUND));
		...
		stockSetReleaseListPrintLogic.updateControlNo(update, tMoveInstH, tMoveInstB, report, errRetSts(StatusCode.UPDATE_EXCEPTION));
		...
		StockSetReleaseListPrintDto stockSetReleaseListPrintDto = new StockSetReleaseListPrintDto();
		stockSetReleaseListPrintDto.output = dto.output;
