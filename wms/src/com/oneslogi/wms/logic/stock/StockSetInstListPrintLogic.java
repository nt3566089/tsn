 ...
 * セット組指示書データ取得ロジッククラス
 */
public class StockSetInstListPrintLogic extends AbstractWmsLogic {
	...
	/**
	 * <h2>在庫移動指示ボディのデータを取得する。</h2>
	 * <pre>
	 * 引数をキーに在庫移動指示ボディを取得する。
	 * 対象が見つからない場合、０件エラーをエラーステータス情報に設定する。
	 *
	 * セット品構成マスタと在庫に紐づく、セット指示指示書のデータを設定する。
	 * </pre>
	 * @param tMoveInstR 在庫移動指示帳票：コントロールNo.
	 * @param errSts エラー時に設定するエラーステータス
	 * @return List<StockSetInstListDto> セット組指示書発行データ取得のリスト
	 */
	public List<StockSetInstListDto> select(TMoveInstR tMoveInstR, ErrorStatus errSts) {
		...
		tMoveInstBCb.query().queryTStoreNo().addOrderBy_StoreLabelNo_Asc();
		...
		// 最後のWMS在庫移動伝票No.のセット組指示書発行データを設定
		// [#3612][Ver3.0] セット組/解除指示書への荷姿単位追加 2018.02.22 honma Mod Start
		setDataList(result, head, bodyList, minimumUnitMap, errSts);
		// [#3612][Ver3.0] セット組/解除指示書への荷姿単位追加 2018.02.22 honma Mod End
	...
	// [#3612][Ver3.0] セット組/解除指示書への荷姿単位追加 2018.02.22 honma Mod Start
	/**
	 * WMS在庫移動伝票No.のセット組指示書発行データを設定編集
	 * @param stockSetInstListDto セット組指示書発行データリスト
	 * @param head セット組指示書のヘッダ
	 * @param bodyList セット組指示書の明細データリスト
	 * @param minimumUnitMap 構成品最小荷姿単位Map
	 * @param errSts エラー時に設定するエラーステータス
	 * @return List<StockSetInstListDto> セット組指示書発行データリスト
	 */
	...
	// [#3612][Ver3.0] セット組/解除指示書への荷姿単位追加 2018.02.22 honma Mod End
		...
		for (TMoveInstB body : bodyList) {
			...
			tStoreNo.setStoreDt(body.chaseTStoreNo().getStoreDt());
			tStoreNo.setStoreLabelNo(body.chaseTStoreNo().getStoreLabelNo());
			...
			dataDto.setStoreDt(CU.nullToStr(body.chaseTStoreNo().getStoreDt()));
			dataDto.setStoreLabelNo(CU.nullToStr(body.chaseTStoreNo().getStoreLabelNo()));
	...
	/**
	 * <h2>セット品構成マスタのデータを取得する。</h2>
	 * <pre>
	 * 引数をキーにセット品構成マスタのデータを取得する。
	 * </pre>
	 ...
	 * @param report 在庫移動指示帳票：指示書出力フラグ
	...
	public void updateControlNo(TMoveInstR update, TMoveInstH tMoveInstH,final TMoveInstB tMoveInstB, TMoveInstR report, ErrorStatus errStatus) {
		...
		cb.query().queryTMoveInstH().existsTMoveInstBList(new SubQuery<TMoveInstBCB>() {
			...
			public void query(TMoveInstBCB subCB) {
				...
				// [ON推-品向-1035] ステータスが完了のデータも発行可能にする修正 2015.12.16 koyama Start
//				subCB.query().setMoveInstStatus_Equal_$01();
				// [ON推-品向-1035] ステータスが完了のデータも発行可能にする修正 2015.12.16 koyama End
				...
				subCB.query().setInoutType_Equal_$0();
		...
		cb.query().queryTMoveInstH().queryMProcessType().setProcessTypeCd_Equal(CDef.ProcessTypeCd.$30.code());
		// [ON推-品向-1035] ステータスが完了のデータも発行可能にする修正 2015.12.16 koyama Start
