...
/**
 * 在庫区分マスタデータの更新ロジッククラス
 */
public class StockTypeMasterUpdateLogic extends AbstractWmsLogic {
	...
	/**
	 * <h2>在庫区分マスタデータの更新処理を行う。</h2>
	 * <pre>
	 * 辞書名存在フラグ = 「0」の場合、辞書マスタを登録し、辞書IDを設定する。
	 * 引数で受け取ったデータで在庫区分マスタに更新する。
	 * </pre>
	 * @param mStockType 在庫区分マスタ：全項目、辞書マスタ：辞書名
	 * @param vDictNmExistFlg 辞書名存在フラグ
	 * @param errSts エラー時に設定するエラーステータス
	 */
	public void update(MStockType mStockType, String vDictNmExistFlg, ErrorStatus errSts) {
		// ===== 在庫区分マスタ更新実行 =====
		...
		mStockTypeBhv.update(mStockType);
	}

	/**
	 * <h2>バッチで在庫区分マスタデータの更新処理を行う。</h2>
	 * <pre>
	 * 引数で受け取ったデータで在庫区分マスタに更新する。
	 * </pre>
	 ...
	 * @param errSts エラー時に設定するエラーステータス
	 */
	public void batchUpdate(List<MStockType> mStockType, ErrorStatus errSts) {
		// ===== 在庫区分マスタ更新実行 =====
		mStockTypeBhv.batchUpdate(mStockType);
	}
