...
・mysqldump_exp.dat　　　処理が実装されているバッチファイル
・mysqldump_exp.ign　　　エクスポート時に除外したいテーブルをリストで指定するファイル
・mysqldump_exp_xxx.bat　mysqldump_expを起動する為のバッチファイル
・mysqldump.env　　　　　既定の接続先を指定するファイル
・mysqldump_exp_xxx.env　mysqldump_exp_xxx.bat単位で接続先を指定する際に準備するファイル
                         ※batファイルと同じ名前のenvファイルは優先される

※mysqldump_exp_xxx.batとmysqldump_exp_xxx.envは複数環境に対応する為に、
...
・mysqldump_imp.dat　　　処理が実装されているバッチファイル
・mysqldump_imp.ign　　　インポート時に除外したいテーブルをリストで指定するファイル
・mysqldump_imp_xxx.bat　mysqldump_impを起動する為のバッチファイル
・mysqldump.env　　　　　既定の接続先を指定するファイル
・mysqldump_imp_xxx.env　mysqldump_imp_xxx.bat単位で接続先を指定する際に準備するファイル
                         ※batファイルと同じ名前のenvファイルは優先される

※mysqldump_imp_xxx.batとmysqldump_imp_xxx.envは複数環境に対応する為に、
　コピーしxxxを変える事でその単位で管理する事が可能になっています。
