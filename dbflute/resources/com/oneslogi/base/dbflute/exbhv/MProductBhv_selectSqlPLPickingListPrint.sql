...
-- [1.1.4-CT-080] 摘み取りリストにバケットNoを表示 2016.06.14 kawana End
 FROM (SELECT    TPH.PICKING_WORK_NO   AS  PICKING_WORK_NO
                ...
                ,MAX(HSH.DELIV_DT) AS DELIV_DT
                ,MAX(HSH.WORK_DT) AS  WORK_DT
...
-- [エンハンス PH2.0] 梱包計算有り時、出庫作業Noを出荷梱包Noに変更 2015.12.03 sja End
-- [1.1.4-CT-080] 摘み取りリストにバケットNoを表示 2016.06.14 kawana Start
                ,MAX(TPAH.CART_NO) AS CART_NO
                ,MAX(TPAH.BUCKET_COL_NO) AS BUCKET_COL_NO
                ,MAX(TPAH.BUCKET_ROW_NO) AS BUCKET_ROW_NO
-- [1.1.4-CT-080] 摘み取りリストにバケットNoを表示 2016.06.14 kawana End
...
-- [エンハンス PH2.0] 梱包計算有り時、出庫作業Noを出荷梱包Noに変更 2015.12.03 sja End
     ...
     LEFT OUTER JOIN M_PRODUCT MP
      ...
      AND MP.DEL_FLG = '0'
      -- [エンハンス PH2.0] ケース荷姿の入数取得を変更 2015.12.03 sja Start
...
--      AND MPS.DEL_FLG = '0'
    ...
    INNER JOIN M_PARAM MPA
	  ...
	  AND MPA.DEL_FLG = '0'
    -- [Ver3.0] unit of measure対応 2017.11.24 REN Del
    -- [エンハンス PH2.0] ケース荷姿の入数取得を変更 2015.12.03 sja End
-- [エンハンス PH2.0] 荷材を追加 2015.12.03 sja Start
     LEFT OUTER JOIN M_BOX MB -- 荷材マスタ
       ON MB.BOX_ID = subA.BOX_ID
      AND MB.DEL_FLG = '0'
-- [エンハンス PH2.0] 荷材を追加 2015.12.03 sja End
    INNER  JOIN T_PICKING_R TPR
  ...
  -- [EC-CT1-171] 再発行はリストの最後に出力(ソート順変更) 2015.04.01 kawana Start
    ORDER BY
             TPR.PL_OUT_FLG
            ,subA.CLIENT_CD
  -- [EC-CT1-171] 再発行はリストの最後に出力(ソート順変更) 2015.04.01 kawana Emd
            ,subA.CENTER_CD
            ,subA.WORK_DT
            -- [#5128][v3.1] 出庫指示バッチNo.を削除しピッキンググループNo.を表示 2018.08.23 kawana Start
            ,subA.PICKING_GROUP_NO
            -- [#5128][v3.1] 出庫指示バッチNo.を削除しピッキンググループNo.を表示 2018.08.23 kawana End
-- [1.1.4-CT-080] 摘み取りリストにバケットNoを表示 2016.06.15 kawana Start
            ,subA.CART_NO
            ,subA.BUCKET_ROW_NO
            ,subA.BUCKET_COL_NO
-- [1.1.4-CT-080] 摘み取りリストにバケットNoを表示 2016.06.15 kawana End
            ,subA.DELIVERY_COURSE_CD
            ,subA.PICKING_WORK_NO
-- [エンハンス PH2.0] 梱包計算有り時、出庫作業Noを出荷梱包Noに変更 2015.12.03 sja Start
            ,subA.SHIPPING_PACKING_NO
