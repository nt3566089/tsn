...
/**
 * The base condition-bean of T_TRRCVDETAIL.
 * @author DBFlute(AutoGenerator)
 */
public class BsTTrrcvdetailCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected TTrrcvdetailCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public BsTTrrcvdetailCB() {
     ...
     * @return this. (NotNull)
     */
    public TTrrcvdetailCB acceptPK(Long receivePlanBId) {
        assertObjectNotNull("receivePlanBId", receivePlanBId);
        BsTTrrcvdetailCB cb = this;
        cb.query().setReceivePlanBId_Equal(receivePlanBId);
        return (TTrrcvdetailCB)this;
    }
     ...
     */
    public TTrrcvdetailCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public TTrrcvdetailCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected TTrrcvdetailCQ doGetConditionQuery() {
    ...
    protected TTrrcvdetailCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected TTrrcvdetailCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        TTrrcvdetailCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected TTrrcvdetailCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new TTrrcvdetailCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<TTrrcvdetailCB> unionCBLambda) {
        final TTrrcvdetailCB cb = new TTrrcvdetailCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final TTrrcvdetailCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<TTrrcvdetailCB> unionCBLambda) {
        final TTrrcvdetailCB cb = new TTrrcvdetailCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final TTrrcvdetailCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
     ...
     *     ... = <span style="color: #553000">tTrrcvdetail</span>.<span style="color: #CC4747">getTTrrcv()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
    ...
    public static class HpSpecification extends HpAbstractSpecification<TTrrcvdetailCQ> {
        ...
        protected void doSpecifyRequiredColumn() {
            ...
            if (qyCall().qy().hasConditionQueryTTrrcv()
                    || qyCall().qy().xgetReferrerQuery() instanceof TTrrcvCQ) {
         ...
         */
        public HpSDRFunction<TTrrcvdetailCB, TTrrcvdetailCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TTrrcvdetailCB> sq, TTrrcvdetailCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public TTrrcvdetailCB dreamCruiseCB() {
        TTrrcvdetailCB cb = new TTrrcvdetailCB();
        cb.xsetupForDreamCruise((TTrrcvdetailCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<TTrrcvdetailCB> columnQuery(final SpecifyQuery<TTrrcvdetailCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected TTrrcvdetailCB xcreateColumnQueryCB() {
        TTrrcvdetailCB cb = new TTrrcvdetailCB();
        cb.xsetupForColumnQuery((TTrrcvdetailCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<TTrrcvdetailCB> orCBLambda) {
        xorSQ((TTrrcvdetailCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<TTrrcvdetailCB> andCBLambda) {
        xorSQAP((TTrrcvdetailCB)this, andCBLambda);
    }
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final TTrrcvdetailCB cb;
        if (mainCB != null) {
            cb = (TTrrcvdetailCB)mainCB;
        } else {
            cb = new TTrrcvdetailCB();
        }
    ...
    protected String xgetConditionBeanClassNameInternally() { return TTrrcvdetailCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return TTrrcvdetailCQ.class.getName(); }
