...
/**
 * The base condition-bean of T_PACKING_B.
 * @author DBFlute(AutoGenerator)
 */
public class BsTPackingBCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected TPackingBCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public BsTPackingBCB() {
     ...
     * @return this. (NotNull)
     */
    public TPackingBCB acceptPK(Long packingBId) {
        assertObjectNotNull("packingBId", packingBId);
        BsTPackingBCB cb = this;
        cb.query().setPackingBId_Equal(packingBId);
        return (TPackingBCB)this;
    }
     ...
     */
    public TPackingBCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public TPackingBCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected TPackingBCQ doGetConditionQuery() {
    ...
    protected TPackingBCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected TPackingBCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        TPackingBCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected TPackingBCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new TPackingBCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<TPackingBCB> unionCBLambda) {
        final TPackingBCB cb = new TPackingBCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final TPackingBCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<TPackingBCB> unionCBLambda) {
        final TPackingBCB cb = new TPackingBCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final TPackingBCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
     ...
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">tPackingB</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">tPackingB</span>.<span style="color: #CC4747">getTPackingH()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
     ...
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">tPackingB</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">tPackingB</span>.<span style="color: #CC4747">getBClassDtlByInspectionFlg()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
    ...
    protected BClassDtlNss _nssBClassDtlByPickingFlg;
    public BClassDtlNss xdfgetNssBClassDtlByPickingFlg() {
        if (_nssBClassDtlByPickingFlg == null) { _nssBClassDtlByPickingFlg = new BClassDtlNss(null); }
        return _nssBClassDtlByPickingFlg;
    }
    /**
     * Set up relation columns to select clause. <br>
     ...
     * <pre>
     * <span style="color: #0000C0">tPackingBBhv</span>.selectEntity(<span style="color: #553000">cb</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     <span style="color: #553000">cb</span>.<span style="color: #CC4747">setupSelect_BClassDtlByPickingFlg()</span>; <span style="color: #3F7E5E">// ...().with[nested-relation]()</span>
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">tPackingB</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">tPackingB</span>.<span style="color: #CC4747">getBClassDtlByPickingFlg()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
    public BClassDtlNss setupSelect_BClassDtlByPickingFlg() {
        assertSetupSelectPurpose("bClassDtlByPickingFlg");
        if (hasSpecifiedLocalColumn()) {
            specify().columnPickingFlg();
        }
        doSetupSelect(() -> query().queryBClassDtlByPickingFlg());
        if (_nssBClassDtlByPickingFlg == null || !_nssBClassDtlByPickingFlg.hasConditionQuery())
        { _nssBClassDtlByPickingFlg = new BClassDtlNss(query().queryBClassDtlByPickingFlg()); }
        return _nssBClassDtlByPickingFlg;
    }
    ...
    public static class HpSpecification extends HpAbstractSpecification<TPackingBCQ> {
         ...
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnCasePickingNo() { return doColumn("CASE_PICKING_NO"); }
        /**
        ...
        protected void doSpecifyRequiredColumn() {
            ...
            if (qyCall().qy().hasConditionQueryBClassDtlByPickingFlg()
                    || qyCall().qy().xgetReferrerQuery() instanceof BClassDtlCQ) {
                columnPickingFlg(); // FK or one-to-one referrer
            }
        }
        @Override
         ...
         * @return The instance for specification for relation table to specify. (NotNull)
         */
        public BClassDtlCB.HpSpecification specifyBClassDtlByPickingFlg() {
            assertRelation("bClassDtlByPickingFlg");
            if (_bClassDtlByPickingFlg == null) {
                _bClassDtlByPickingFlg = new BClassDtlCB.HpSpecification(_baseCB
                    , xcreateSpQyCall(() -> _qyCall.has() && _qyCall.qy().hasConditionQueryBClassDtlByPickingFlg()
                                    , () -> _qyCall.qy().queryBClassDtlByPickingFlg())
                    , _purpose, _dbmetaProvider, xgetSDRFnFc());
                if (xhasSyncQyCall()) { // inherits it
                    _bClassDtlByPickingFlg.xsetSyncQyCall(xcreateSpQyCall(
                        () -> xsyncQyCall().has() && xsyncQyCall().qy().hasConditionQueryBClassDtlByPickingFlg()
                      , () -> xsyncQyCall().qy().queryBClassDtlByPickingFlg()));
                }
            }
         ...
         */
        public HpSDRFunction<TPackingBCB, TPackingBCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TPackingBCB> sq, TPackingBCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public TPackingBCB dreamCruiseCB() {
        TPackingBCB cb = new TPackingBCB();
        cb.xsetupForDreamCruise((TPackingBCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<TPackingBCB> columnQuery(final SpecifyQuery<TPackingBCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected TPackingBCB xcreateColumnQueryCB() {
        TPackingBCB cb = new TPackingBCB();
        cb.xsetupForColumnQuery((TPackingBCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<TPackingBCB> orCBLambda) {
        xorSQ((TPackingBCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<TPackingBCB> andCBLambda) {
        xorSQAP((TPackingBCB)this, andCBLambda);
    }
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final TPackingBCB cb;
        if (mainCB != null) {
            cb = (TPackingBCB)mainCB;
        } else {
            cb = new TPackingBCB();
        }
    ...
    protected String xgetConditionBeanClassNameInternally() { return TPackingBCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return TPackingBCQ.class.getName(); }
