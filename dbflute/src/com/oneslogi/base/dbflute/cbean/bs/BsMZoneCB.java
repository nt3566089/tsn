...
/**
 * The base condition-bean of M_ZONE.
 * @author DBFlute(AutoGenerator)
 */
public class BsMZoneCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected MZoneCQ _conditionQuery;
     ...
     * @return this. (NotNull)
     */
    public MZoneCB acceptPK(Long zoneId) {
     ...
     * @param zoneId : PK, ID, NotNull, bigint identity(19). (NotNull)
     */
    public void acceptPrimaryKey(Long zoneId) {
        assertObjectNotNull("zoneId", zoneId);
        BsMZoneCB cb = this;
        cb.query().setZoneId_Equal(zoneId);
    }
     ...
     * @return this. (NotNull)
     */
    public MZoneCB acceptUniqueOf(Long warehouseId, String zoneCd) {
        assertObjectNotNull("warehouseId", warehouseId);assertObjectNotNull("zoneCd", zoneCd);
        BsMZoneCB cb = this;
        cb.query().setWarehouseId_Equal(warehouseId);cb.query().setZoneCd_Equal(zoneCd);
        return (MZoneCB)this;
    }

    public ConditionBean addOrderBy_PK_Asc() {
        query().addOrderBy_ZoneId_Asc();
        return this;
    }

    public ConditionBean addOrderBy_PK_Desc() {
        query().addOrderBy_ZoneId_Desc();
        return this;
    }
     ...
     */
    public MZoneCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public MZoneCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected MZoneCQ doGetConditionQuery() {
    ...
    protected MZoneCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected MZoneCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        MZoneCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected MZoneCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new MZoneCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<MZoneCB> unionCBLambda) {
        final MZoneCB cb = new MZoneCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final MZoneCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<MZoneCB> unionCBLambda) {
        final MZoneCB cb = new MZoneCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final MZoneCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
     ...
     * <pre>
     * <span style="color: #0000C0">mZoneBhv</span>.selectEntity(<span style="color: #553000">cb</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     ...
     * }).alwaysPresent(<span style="color: #553000">mZone</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">mZone</span>.<span style="color: #CC4747">getMWarehouse()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     ...
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">mZone</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">mZone</span>.<span style="color: #CC4747">getBClassDtlByDelFlg()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
    ...
    public static class HpSpecification extends HpAbstractSpecification<MZoneCQ> {
        protected MWarehouseCB.HpSpecification _mWarehouse;
        protected BClassDtlCB.HpSpecification _bClassDtlByDelFlg;
        public HpSpecification(ConditionBean baseCB, HpSpQyCall<MZoneCQ> qyCall
        ...
        @Override
        protected String getTableDbName() { return "M_ZONE"; }
        /**
         ...
         */
        public HpSDRFunction<MLocationCB, MZoneCQ> derivedMLocationList() {
            assertDerived("mLocationList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<MLocationCB> sq, MZoneCQ cq, String al, DerivedReferrerOption op)
         ...
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<MParamCB, MZoneCQ> derivedMParamList() {
            assertDerived("mParamList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<MParamCB> sq, MZoneCQ cq, String al, DerivedReferrerOption op)
         ...
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<TInventoryInstCB, MZoneCQ> derivedTInventoryInstList() {
            assertDerived("tInventoryInstList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TInventoryInstCB> sq, MZoneCQ cq, String al, DerivedReferrerOption op)
         ...
         */
        public HpSDRFunction<MZoneCB, MZoneCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<MZoneCB> sq, MZoneCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public MZoneCB dreamCruiseCB() {
        MZoneCB cb = new MZoneCB();
        cb.xsetupForDreamCruise((MZoneCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<MZoneCB> columnQuery(final SpecifyQuery<MZoneCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected MZoneCB xcreateColumnQueryCB() {
        MZoneCB cb = new MZoneCB();
        cb.xsetupForColumnQuery((MZoneCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<MZoneCB> orCBLambda) {
        xorSQ((MZoneCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<MZoneCB> andCBLambda) {
        xorSQAP((MZoneCB)this, andCBLambda);
    }
    ...
    protected String xgetConditionBeanClassNameInternally() { return MZoneCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return MZoneCQ.class.getName(); }
