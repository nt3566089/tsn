...
/**
 * The base condition-bean of T_PALLET.
 * @author DBFlute(AutoGenerator)
 */
public class BsTPalletCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected TPalletCQ _conditionQuery;
     ...
     * @return this. (NotNull)
     */
    public TPalletCB acceptPK(Long palletId) {
        assertObjectNotNull("palletId", palletId);
        BsTPalletCB cb = this;
        cb.query().setPalletId_Equal(palletId);
        return (TPalletCB)this;
    }
     ...
     * @param palletId : PK, ID, NotNull, bigint identity(19). (NotNull)
     */
    public void acceptPrimaryKey(Long palletId) {
        assertObjectNotNull("palletId", palletId);
        BsTPalletCB cb = this;
        cb.query().setPalletId_Equal(palletId);
    }
    ...
    public ConditionBean addOrderBy_PK_Asc() {
        query().addOrderBy_PalletId_Asc();
        return this;
    }

    public ConditionBean addOrderBy_PK_Desc() {
        query().addOrderBy_PalletId_Desc();
        return this;
    }
     ...
     */
    public TPalletCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public TPalletCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected TPalletCQ doGetConditionQuery() {
    ...
    protected TPalletCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected TPalletCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        TPalletCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected TPalletCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new TPalletCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<TPalletCB> unionCBLambda) {
        final TPalletCB cb = new TPalletCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final TPalletCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<TPalletCB> unionCBLambda) {
        final TPalletCB cb = new TPalletCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final TPalletCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
     ...
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">tPallet</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">tPallet</span>.<span style="color: #CC4747">getMCenter()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
     ...
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">tPallet</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">tPallet</span>.<span style="color: #CC4747">getTStock()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
    ...
    public static class HpSpecification extends HpAbstractSpecification<TPalletCQ> {
         ...
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnDummyPalletKbn() { return doColumn("DUMMY_PALLET_KBN"); }
        /**
         ...
         */
        public HpSDRFunction<TCenterSymbolCB, TPalletCQ> derivedTCenterSymbolList() {
            assertDerived("tCenterSymbolList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TCenterSymbolCB> sq, TPalletCQ cq, String al, DerivedReferrerOption op)
         ...
         */
        public HpSDRFunction<TPalletCB, TPalletCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TPalletCB> sq, TPalletCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public TPalletCB dreamCruiseCB() {
        TPalletCB cb = new TPalletCB();
        cb.xsetupForDreamCruise((TPalletCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<TPalletCB> columnQuery(final SpecifyQuery<TPalletCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected TPalletCB xcreateColumnQueryCB() {
        TPalletCB cb = new TPalletCB();
        cb.xsetupForColumnQuery((TPalletCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<TPalletCB> orCBLambda) {
        xorSQ((TPalletCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<TPalletCB> andCBLambda) {
        xorSQAP((TPalletCB)this, andCBLambda);
    }
    ...
    protected String xgetConditionBeanClassNameInternally() { return TPalletCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return TPalletCQ.class.getName(); }
