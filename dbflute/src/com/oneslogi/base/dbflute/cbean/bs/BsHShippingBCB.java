...
/**
 * The base condition-bean of H_SHIPPING_B.
 * @author DBFlute(AutoGenerator)
 */
public class BsHShippingBCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected HShippingBCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public BsHShippingBCB() {
     ...
     */
    public HShippingBCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public HShippingBCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected HShippingBCQ doGetConditionQuery() {
    ...
    protected HShippingBCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected HShippingBCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        HShippingBCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected HShippingBCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new HShippingBCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<HShippingBCB> unionCBLambda) {
        final HShippingBCB cb = new HShippingBCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final HShippingBCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<HShippingBCB> unionCBLambda) {
        final HShippingBCB cb = new HShippingBCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final HShippingBCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
     ...
     *     <span style="color: #553000">cb</span>.<span style="color: #CC4747">setupSelect_HShippingH()</span>; <span style="color: #3F7E5E">// ...().with[nested-relation]()</span>
     ...
     *     ... = <span style="color: #553000">hShippingB</span>.<span style="color: #CC4747">getHShippingH()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
    public HShippingHNss setupSelect_HShippingH() {
        assertSetupSelectPurpose("hShippingH");
        if (hasSpecifiedLocalColumn()) {
            specify().columnShippingInstHId();
        }
        doSetupSelect(() -> query().queryHShippingH());
    ...
    public HShippingSpareNss xdfgetNssHShippingSpareAsOne() {
        if (_nssHShippingSpareAsOne == null) { _nssHShippingSpareAsOne = new HShippingSpareNss(null); }
        return _nssHShippingSpareAsOne;
    }
    /**
     * Set up relation columns to select clause. <br>
     ...
     * <pre>
     * <span style="color: #0000C0">hShippingBBhv</span>.selectEntity(<span style="color: #553000">cb</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     <span style="color: #553000">cb</span>.<span style="color: #CC4747">setupSelect_HShippingSpareAsOne()</span>; <span style="color: #3F7E5E">// ...().with[nested-relation]()</span>
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">hShippingB</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">hShippingB</span>.<span style="color: #CC4747">getHShippingSpareAsOne()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
    public HShippingSpareNss setupSelect_HShippingSpareAsOne() {
        assertSetupSelectPurpose("hShippingSpareAsOne");
        doSetupSelect(() -> query().queryHShippingSpareAsOne());
        if (_nssHShippingSpareAsOne == null || !_nssHShippingSpareAsOne.hasConditionQuery())
        { _nssHShippingSpareAsOne = new HShippingSpareNss(query().queryHShippingSpareAsOne()); }
        return _nssHShippingSpareAsOne;
    }
    ...
    public static class HpSpecification extends HpAbstractSpecification<HShippingBCQ> {
         ...
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnShapeNm() { return doColumn("SHAPE_NM"); }
        /**
         * UNIT_NUM: {bigint(19)}
         * @return The information object of specified column. (NotNull)
         */
         ...
         */
        public HShippingHCB.HpSpecification specifyHShippingH() {
            assertRelation("hShippingH");
            if (_hShippingH == null) {
                _hShippingH = new HShippingHCB.HpSpecification(_baseCB
                    , xcreateSpQyCall(() -> _qyCall.has() && _qyCall.qy().hasConditionQueryHShippingH()
                                    , () -> _qyCall.qy().queryHShippingH())
                    , _purpose, _dbmetaProvider, xgetSDRFnFc());
                if (xhasSyncQyCall()) { // inherits it
                    _hShippingH.xsetSyncQyCall(xcreateSpQyCall(
                        () -> xsyncQyCall().has() && xsyncQyCall().qy().hasConditionQueryHShippingH()
                      , () -> xsyncQyCall().qy().queryHShippingH()));
                }
            }
            return _hShippingH;
        }
         ...
         * @return The instance for specification for relation table to specify. (NotNull)
         */
        public HShippingSpareCB.HpSpecification specifyHShippingSpareAsOne() {
            assertRelation("hShippingSpareAsOne");
            if (_hShippingSpareAsOne == null) {
                _hShippingSpareAsOne = new HShippingSpareCB.HpSpecification(_baseCB
                    , xcreateSpQyCall(() -> _qyCall.has() && _qyCall.qy().hasConditionQueryHShippingSpareAsOne()
                                    , () -> _qyCall.qy().queryHShippingSpareAsOne())
                    , _purpose, _dbmetaProvider, xgetSDRFnFc());
                if (xhasSyncQyCall()) { // inherits it
                    _hShippingSpareAsOne.xsetSyncQyCall(xcreateSpQyCall(
                        () -> xsyncQyCall().has() && xsyncQyCall().qy().hasConditionQueryHShippingSpareAsOne()
                      , () -> xsyncQyCall().qy().queryHShippingSpareAsOne()));
                }
            }
         ...
         */
        public HpSDRFunction<HPackingBCB, HShippingBCQ> derivedHPackingBList() {
            assertDerived("hPackingBList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<HPackingBCB> sq, HShippingBCQ cq, String al, DerivedReferrerOption op)
         ...
         */
        public HpSDRFunction<HShippingBCB, HShippingBCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<HShippingBCB> sq, HShippingBCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public HShippingBCB dreamCruiseCB() {
        HShippingBCB cb = new HShippingBCB();
        cb.xsetupForDreamCruise((HShippingBCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<HShippingBCB> columnQuery(final SpecifyQuery<HShippingBCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected HShippingBCB xcreateColumnQueryCB() {
        HShippingBCB cb = new HShippingBCB();
        cb.xsetupForColumnQuery((HShippingBCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<HShippingBCB> orCBLambda) {
        xorSQ((HShippingBCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<HShippingBCB> andCBLambda) {
        xorSQAP((HShippingBCB)this, andCBLambda);
    }
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final HShippingBCB cb;
        if (mainCB != null) {
            cb = (HShippingBCB)mainCB;
        } else {
            cb = new HShippingBCB();
        }
    ...
    protected String xgetConditionBeanClassNameInternally() { return HShippingBCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return HShippingBCQ.class.getName(); }
