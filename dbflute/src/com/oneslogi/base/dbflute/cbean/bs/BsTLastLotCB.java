...
/**
 * The base condition-bean of T_LAST_LOT.
 * @author DBFlute(AutoGenerator)
 */
public class BsTLastLotCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected TLastLotCQ _conditionQuery;
     ...
     * @return this. (NotNull)
     */
    public TLastLotCB acceptPK(Long lastLotId) {
        assertObjectNotNull("lastLotId", lastLotId);
        BsTLastLotCB cb = this;
        cb.query().setLastLotId_Equal(lastLotId);
        return (TLastLotCB)this;
    }
     ...
     * @param lastLotId : PK, ID, NotNull, bigint identity(19). (NotNull)
     */
    public void acceptPrimaryKey(Long lastLotId) {
        assertObjectNotNull("lastLotId", lastLotId);
        BsTLastLotCB cb = this;
        cb.query().setLastLotId_Equal(lastLotId);
    }
     ...
     * @return this. (NotNull)
     */
    public TLastLotCB acceptUniqueOf(Long customerId, Long productId) {
        assertObjectNotNull("customerId", customerId);assertObjectNotNull("productId", productId);
        BsTLastLotCB cb = this;
        cb.query().setCustomerId_Equal(customerId);cb.query().setProductId_Equal(productId);
        return (TLastLotCB)this;
    }

    public ConditionBean addOrderBy_PK_Asc() {
        query().addOrderBy_LastLotId_Asc();
        return this;
    }

    public ConditionBean addOrderBy_PK_Desc() {
        query().addOrderBy_LastLotId_Desc();
        return this;
    }
     ...
     */
    public TLastLotCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public TLastLotCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected TLastLotCQ doGetConditionQuery() {
    ...
    protected TLastLotCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected TLastLotCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        TLastLotCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected TLastLotCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new TLastLotCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<TLastLotCB> unionCBLambda) {
        final TLastLotCB cb = new TLastLotCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final TLastLotCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<TLastLotCB> unionCBLambda) {
        final TLastLotCB cb = new TLastLotCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final TLastLotCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
     ...
     *     ... = <span style="color: #553000">tLastLot</span>.<span style="color: #CC4747">getMCustomer()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     ...
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">tLastLot</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">tLastLot</span>.<span style="color: #CC4747">getTLot()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
    ...
    public static class HpSpecification extends HpAbstractSpecification<TLastLotCQ> {
         ...
         */
        public SpecifiedColumn columnLastLotId() { return doColumn("LAST_LOT_ID"); }
        /**
        ...
        @Override
        protected String getTableDbName() { return "T_LAST_LOT"; }
        /**
         ...
         */
        public HpSDRFunction<TLastLotCB, TLastLotCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TLastLotCB> sq, TLastLotCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public TLastLotCB dreamCruiseCB() {
        TLastLotCB cb = new TLastLotCB();
        cb.xsetupForDreamCruise((TLastLotCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<TLastLotCB> columnQuery(final SpecifyQuery<TLastLotCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected TLastLotCB xcreateColumnQueryCB() {
        TLastLotCB cb = new TLastLotCB();
        cb.xsetupForColumnQuery((TLastLotCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<TLastLotCB> orCBLambda) {
        xorSQ((TLastLotCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<TLastLotCB> andCBLambda) {
        xorSQAP((TLastLotCB)this, andCBLambda);
    }
    ...
    protected String xgetConditionBeanClassNameInternally() { return TLastLotCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return TLastLotCQ.class.getName(); }
