...
/**
 * The base condition-bean of T_CORDDTAAM.
 * @author DBFlute(AutoGenerator)
 */
public class BsTCorddtaamCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected TCorddtaamCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public BsTCorddtaamCB() {
     ...
     * @return this. (NotNull)
     */
    public TCorddtaamCB acceptPK(Long orderRecordId) {
        assertObjectNotNull("orderRecordId", orderRecordId);
        BsTCorddtaamCB cb = this;
        cb.query().setOrderRecordId_Equal(orderRecordId);
        return (TCorddtaamCB)this;
    }
     ...
     * @param orderRecordId : PK, ID, NotNull, bigint identity(19). (NotNull)
     */
    public void acceptPrimaryKey(Long orderRecordId) {
        assertObjectNotNull("orderRecordId", orderRecordId);
        BsTCorddtaamCB cb = this;
        cb.query().setOrderRecordId_Equal(orderRecordId);
    }
    ...
    public ConditionBean addOrderBy_PK_Asc() {
        query().addOrderBy_OrderRecordId_Asc();
        return this;
    }

    public ConditionBean addOrderBy_PK_Desc() {
        query().addOrderBy_OrderRecordId_Desc();
        return this;
    }
     ...
     */
    public TCorddtaamCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public TCorddtaamCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected TCorddtaamCQ doGetConditionQuery() {
    ...
    protected TCorddtaamCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected TCorddtaamCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        TCorddtaamCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected TCorddtaamCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new TCorddtaamCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<TCorddtaamCB> unionCBLambda) {
        final TCorddtaamCB cb = new TCorddtaamCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final TCorddtaamCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<TCorddtaamCB> unionCBLambda) {
        final TCorddtaamCB cb = new TCorddtaamCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final TCorddtaamCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
    ...
    public static class HpSpecification extends HpAbstractSpecification<TCorddtaamCQ> {
        public HpSpecification(ConditionBean baseCB, HpSpQyCall<TCorddtaamCQ> qyCall
         ...
         */
        public SpecifiedColumn columnOrderRecordId() { return doColumn("ORDER_RECORD_ID"); }
        /**
        ...
        protected void doSpecifyRequiredColumn() {
            columnOrderRecordId(); // PK
        }
        @Override
        protected String getTableDbName() { return "T_CORDDTAAM"; }
        /**
         * Prepare for (Specify)MyselfDerived (SubQuery).
         * @return The object to set up a function for myself table. (NotNull)
         */
        public HpSDRFunction<TCorddtaamCB, TCorddtaamCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TCorddtaamCB> sq, TCorddtaamCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public TCorddtaamCB dreamCruiseCB() {
        TCorddtaamCB cb = new TCorddtaamCB();
        cb.xsetupForDreamCruise((TCorddtaamCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<TCorddtaamCB> columnQuery(final SpecifyQuery<TCorddtaamCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected TCorddtaamCB xcreateColumnQueryCB() {
        TCorddtaamCB cb = new TCorddtaamCB();
        cb.xsetupForColumnQuery((TCorddtaamCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<TCorddtaamCB> orCBLambda) {
        xorSQ((TCorddtaamCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<TCorddtaamCB> andCBLambda) {
        xorSQAP((TCorddtaamCB)this, andCBLambda);
    }
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final TCorddtaamCB cb;
        if (mainCB != null) {
            cb = (TCorddtaamCB)mainCB;
        } else {
            cb = new TCorddtaamCB();
        }
    ...
    protected String xgetConditionBeanClassNameInternally() { return TCorddtaamCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return TCorddtaamCQ.class.getName(); }
