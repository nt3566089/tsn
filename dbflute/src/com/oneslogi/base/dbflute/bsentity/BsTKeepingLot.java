...
/**
 * The entity of T_KEEPING_LOT as TABLE. <br>
 * 保管ロット管理
 * <pre>
 * [primary-key]
 *     KEEPING_LOT_ID
 ...
 * [get/set template]
 * /= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
 * Long keepingLotId = entity.getKeepingLotId();
 ...
 * entity.setKeepingLotId(keepingLotId);
 ...
 */
public abstract class BsTKeepingLot extends AbstractEntity implements DomainEntity, EntityDefinedCommonColumn {
    ...
    public boolean hasPrimaryKeyValue() {
        if (_keepingLotId == null) { return false; }
    ...
    protected boolean doEquals(Object obj) {
        if (obj instanceof BsTKeepingLot) {
            BsTKeepingLot other = (BsTKeepingLot)obj;
            if (!xSV(_keepingLotId, other._keepingLotId)) { return false; }
            return true;
        } else {
            return false;
        }
    ...
    protected String doBuildColumnString(String dm) {
        StringBuilder sb = new StringBuilder();
        sb.append(dm).append(xfND(_keepingLotId));
    ...
    @Override
    public TKeepingLot clone() {
        return (TKeepingLot)super.clone();
    }
    ...
    //                                                                            ========
    /**
     * [get] KEEPING_LOT_ID: {PK, ID, NotNull, bigint identity(19)} <br>
     * 保管ロットID
     * @return The value of the column 'KEEPING_LOT_ID'. (basically NotNull if selected: for the constraint)
     */
    public Long getKeepingLotId() {
        checkSpecifiedProperty("keepingLotId");
        return _keepingLotId;
    }

    /**
     * [set] KEEPING_LOT_ID: {PK, ID, NotNull, bigint identity(19)} <br>
     * 保管ロットID
     * @param keepingLotId The value of the column 'KEEPING_LOT_ID'. (basically NotNull if update: for the constraint)
     */
    public void setKeepingLotId(Long keepingLotId) {
        registerModifiedProperty("keepingLotId");
        _keepingLotId = keepingLotId;
    }

    /**
     * [get] CUSTOMER_ID: {IX, NotNull, bigint(19), FK to M_CUSTOMER} <br>
     * 取引先ID
    ...
    /**
     * [set] CUSTOMER_ID: {IX, NotNull, bigint(19), FK to M_CUSTOMER} <br>
     * 取引先ID
