...
/**
 * The entity of W_HT_SERIAL_RECEIVE_INSP as TABLE. <br>
 * シリアル登録(入荷)ワーク
 * <pre>
 * [primary-key]
 ...
 * [get/set template]
 * /= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
 * Long htSerialReceiveInspId = entity.getHtSerialReceiveInspId();
 ...
 * entity.setHtSerialReceiveInspId(htSerialReceiveInspId);
 ...
 */
public abstract class BsWHtSerialReceiveInsp extends AbstractEntity implements DomainEntity, EntityDefinedCommonColumn {
    ...
    protected Long _htSerialReceiveInspId;
    ...
    public boolean hasPrimaryKeyValue() {
        if (_htSerialReceiveInspId == null) { return false; }
    ...
    protected boolean doEquals(Object obj) {
        if (obj instanceof BsWHtSerialReceiveInsp) {
            BsWHtSerialReceiveInsp other = (BsWHtSerialReceiveInsp)obj;
            if (!xSV(_htSerialReceiveInspId, other._htSerialReceiveInspId)) { return false; }
            return true;
        } else {
            return false;
        }
    ...
    protected int doHashCode(int initial) {
        ...
        hs = xCH(hs, _htSerialReceiveInspId);
        return hs;
    }
    ...
    protected String doBuildColumnString(String dm) {
        StringBuilder sb = new StringBuilder();
        sb.append(dm).append(xfND(_htSerialReceiveInspId));
    ...
    @Override
    public WHtSerialReceiveInsp clone() {
        return (WHtSerialReceiveInsp)super.clone();
    }
    ...
    //                                                                            ========
    /**
     * [get] HT_SERIAL_RECEIVE_INSP_ID: {PK, ID, NotNull, bigint identity(19)} <br>
     * シリアル登録(入荷)ワークID
     * @return The value of the column 'HT_SERIAL_RECEIVE_INSP_ID'. (basically NotNull if selected: for the constraint)
     */
    public Long getHtSerialReceiveInspId() {
        checkSpecifiedProperty("htSerialReceiveInspId");
        return _htSerialReceiveInspId;
    }

    /**
     * [set] HT_SERIAL_RECEIVE_INSP_ID: {PK, ID, NotNull, bigint identity(19)} <br>
     * シリアル登録(入荷)ワークID
     * @param htSerialReceiveInspId The value of the column 'HT_SERIAL_RECEIVE_INSP_ID'. (basically NotNull if update: for the constraint)
     */
    public void setHtSerialReceiveInspId(Long htSerialReceiveInspId) {
        registerModifiedProperty("htSerialReceiveInspId");
        _htSerialReceiveInspId = htSerialReceiveInspId;
    }
